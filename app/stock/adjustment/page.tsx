...
export default function AdjustmentPage() {
  ...
  const handleSubmit = async () => {
    if (!form.itemid || !form.warehouseid || !form.adjustmentquantity) { showToast("Please fill all required fields", "error"); return; }
    setLoading(true);
    try {
      const res = await fetch("/api/stock/adjustment", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...form, adjustmentquantity: Number(form.adjustmentquantity) }) });
      if (!res.ok) throw new Error();
      showToast("Adjustment recorded!");
      setForm({ itemid: "", warehouseid: "", batchid: "", adjustmentquantity: "", reason: "", createdby: "" });
  ...
  const inp = (label: string, key: keyof typeof form, type = "text", required = false, hint?: string) => (
    <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
      <label style={{ fontSize: 12, fontWeight: 600, color: "#374151", textTransform: "uppercase", letterSpacing: "0.05em" }}>{label} {required && <span style={{ color: "#dc2626" }}>*</span>}</label>
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f1f5f9", fontFamily: "'Inter', system-ui, sans-serif" }}>
      ...
      <div style={{ background: "#fff", borderBottom: "1px solid #e5e7eb", padding: "0 32px", height: 64, display: "flex", alignItems: "center", gap: 16, position: "sticky", top: 0, zIndex: 100 }}>
        ...
        <div style={{ fontSize: 15, fontWeight: 700, color: "#111827" }}>Stock Adjustment</div>
      </div>
      <div style={{ padding: "32px", maxWidth: 720, margin: "0 auto" }}>
        <div style={{ marginBottom: 24 }}>
          <h1 style={{ margin: 0, fontSize: 22, fontWeight: 700, color: "#111827" }}>Stock Adjustment</h1>
          <p style={{ margin: "4px 0 0", fontSize: 13, color: "#6b7280" }}>Correct stock levels during physical counts</p>
        </div>
        <div style={{ background: "#fff", borderRadius: 14, border: "1px solid #f3f4f6", padding: "28px 32px" }}>
          ...
          <div style={{ marginTop: 24, display: "flex", gap: 12, justifyContent: "flex-end" }}>
            ...
            <button onClick={handleSubmit} disabled={loading} style={{ padding: "10px 24px", border: "none", borderRadius: 8, background: loading ? "#93c5fd" : "#d97706", color: "#fff", fontSize: 13, fontWeight: 600, cursor: loading ? "not-allowed" : "pointer" }}>
