...
export default function ReceiveStockPage() {
  ...
  const handleSubmit = async () => {
    if (!form.itemid || !form.warehouseid || !form.batchnumber || !form.quantity) {
    ...
    try {
      ...
      setForm({ itemid: "", warehouseid: "", batchnumber: "", manufacturedate: "", expirydate: "", quantity: "", notes: "", createdby: "" });
    } catch { showToast("Failed to receive stock", "error"); }
    finally { setLoading(false); }
  };

  const field = (label: string, key: keyof typeof form, type = "text", required = false, options?: { value: string; label: string }[]) => (
    ...
    <div style={{ minHeight: "100vh", background: "#f1f5f9", fontFamily: "'Inter', system-ui, sans-serif" }}>
      ...
      <div style={{ padding: "32px", maxWidth: 720, margin: "0 auto" }}>
        <div style={{ marginBottom: 24 }}>
          <h1 style={{ margin: 0, fontSize: 22, fontWeight: 700, color: "#111827" }}>Receive Stock</h1>
          <p style={{ margin: "4px 0 0", fontSize: 13, color: "#6b7280" }}>Record incoming inventory from suppliers</p>
        </div>
        ...
        <div style={{ background: "#fff", borderRadius: 14, border: "1px solid #f3f4f6", padding: "28px 32px" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18 }}>
            ...
            {field("Manufacture Date", "manufacturedate", "date")}
            {field("Expiry Date", "expirydate", "date")}
            {field("Received By", "createdby")}
            <div style={{ gridColumn: "1/-1" }}>{field("Notes", "notes")}</div>
          </div>

          <div style={{ marginTop: 24, display: "flex", gap: 12, justifyContent: "flex-end" }}>
            <button onClick={() => router.push("/stock")}
              style={{ padding: "10px 20px", border: "1px solid #e5e7eb", borderRadius: 8, background: "#fff", fontSize: 13, fontWeight: 600, cursor: "pointer", color: "#374151" }}>
              Cancel
            </button>
            <button onClick={handleSubmit} disabled={loading}
              style={{ padding: "10px 24px", border: "none", borderRadius: 8, background: loading ? "#93c5fd" : "#16a34a", color: "#fff", fontSize: 13, fontWeight: 600, cursor: loading ? "not-allowed" : "pointer" }}>
