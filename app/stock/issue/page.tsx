...
export default function IssueStockPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setForm({ itemid: "", warehouseid: "", batchid: "", quantity: "", departmentid: "", issuedby: "", notes: "" });
    } catch (e: any) { showToast(e.message || "Failed to issue stock", "error"); }
    finally { setLoading(false); }
  };

  const select = (label: string, key: keyof typeof form, required = false, options: { value: string; label: string }[]) => (
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f1f5f9", fontFamily: "'Inter', system-ui, sans-serif" }}>
      ...
      <div style={{ background: "#fff", borderBottom: "1px solid #e5e7eb", padding: "0 32px", height: 64, display: "flex", alignItems: "center", gap: 16, position: "sticky", top: 0, zIndex: 100 }}>
        ...
        <div style={{ fontSize: 15, fontWeight: 700, color: "#111827" }}>Issue Stock</div>
      </div>

      <div style={{ padding: "32px", maxWidth: 720, margin: "0 auto" }}>
        <div style={{ marginBottom: 24 }}>
          <h1 style={{ margin: 0, fontSize: 22, fontWeight: 700, color: "#111827" }}>Issue Stock</h1>
          <p style={{ margin: "4px 0 0", fontSize: 13, color: "#6b7280" }}>Dispatch items to hospital departments</p>
        </div>
