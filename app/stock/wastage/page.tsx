...
const icons = { back: "M19 12H5M12 5l-7 7 7 7", check: "M20 6L9 17l-5-5", x: "M18 6L6 18M6 6l12 12" };

const WASTAGE_REASONS = ["Expired", "Damaged", "Lost", "Contaminated", "Other"];
...
export default function WastagePage() {
  ...
  const [form, setForm] = useState({ itemid: "", warehouseid: "", batchid: "", quantity: "", reason: "", createdby: "" });
  ...
  const handleSubmit = async () => {
    if (!form.itemid || !form.warehouseid || !form.quantity || !form.reason) { showToast("Please fill all required fields", "error"); return; }
    setLoading(true);
    try {
      const res = await fetch("/api/stock/wastage", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...form, quantity: Number(form.quantity) }) });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error);
      showToast("Wastage recorded!");
      setForm({ itemid: "", warehouseid: "", batchid: "", quantity: "", reason: "", createdby: "" });
    } catch (e: any) { showToast(e.message || "Failed", "error"); } finally { setLoading(false); }
  };
  ...
  const sel = (label: string, key: keyof typeof form, required = false, options: { value: string; label: string }[]) => (
    <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
      ...
      <select value={form[key]} onChange={e => setForm(f => ({ ...f, [key]: e.target.value }))} style={{ padding: "9px 12px", border: "1px solid #e5e7eb", borderRadius: 8, fontSize: 13, background: "#f9fafb", outline: "none", color: "#111827" }}>
  ...
  const inp = (label: string, key: keyof typeof form, type = "text", required = false) => (
    <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
      <label style={{ fontSize: 12, fontWeight: 600, color: "#374151", textTransform: "uppercase", letterSpacing: "0.05em" }}>{label} {required && <span style={{ color: "#dc2626" }}>*</span>}</label>
      <input type={type} value={form[key]} onChange={e => setForm(f => ({ ...f, [key]: e.target.value }))} style={{ padding: "9px 12px", border: "1px solid #e5e7eb", borderRadius: 8, fontSize: 13, background: "#f9fafb", outline: "none", color: "#111827" }} />
    </div>
  );
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f1f5f9", fontFamily: "'Inter', system-ui, sans-serif" }}>
      ...
      <div style={{ background: "#fff", borderBottom: "1px solid #e5e7eb", padding: "0 32px", height: 64, display: "flex", alignItems: "center", gap: 16, position: "sticky", top: 0, zIndex: 100 }}>
        ...
        <div style={{ fontSize: 15, fontWeight: 700, color: "#111827" }}>Record Wastage</div>
      </div>
      <div style={{ padding: "32px", maxWidth: 720, margin: "0 auto" }}>
        <div style={{ marginBottom: 24 }}>
          <h1 style={{ margin: 0, fontSize: 22, fontWeight: 700, color: "#111827" }}>Record Wastage</h1>
          <p style={{ margin: "4px 0 0", fontSize: 13, color: "#6b7280" }}>Log expired, damaged or lost items</p>
        </div>
        <div style={{ background: "#fff", borderRadius: 14, border: "1px solid #f3f4f6", padding: "28px 32px" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18 }}>
            ...
            {sel("Reason", "reason", true, WASTAGE_REASONS.map(r => ({ value: r, label: r })))}
            {inp("Recorded By", "createdby")}
          </div>
          <div style={{ marginTop: 24, display: "flex", gap: 12, justifyContent: "flex-end" }}>
            ...
            <button onClick={handleSubmit} disabled={loading} style={{ padding: "10px 24px", border: "none", borderRadius: 8, background: loading ? "#93c5fd" : "#dc2626", color: "#fff", fontSize: 13, fontWeight: 600, cursor: loading ? "not-allowed" : "pointer" }}>
              {loading ? "Saving..." : "Record Wastage"}
            </button>
          </div>
        </div>
      </div>
      {toast && <div style={{ position: "fixed", bottom: 24, right: 24, background: toast.type === "error" ? "#dc2626" : "#16a34a", color: "#fff", padding: "12px 20px", borderRadius: 10, fontSize: 13, fontWeight: 600, boxShadow: "0 8px 24px rgba(0,0,0,0.15)", zIndex: 2000, display: "flex", alignItems: "center", gap: 8 }}>
