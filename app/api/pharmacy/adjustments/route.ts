...
export async function GET(req: NextRequest) {
  ...
  const result = await pool.query(
    `SELECT
      sa.id,
      sa.adjustment_quantity AS "adjustmentQty",
      sa.reason,
...
export async function POST(req: NextRequest) {
  ...
  await pool.query(
    `INSERT INTO stock_adjustments (id, item_id, warehouse_id, batch_id, adjustment_quantity, reason, created_by, created_at)
