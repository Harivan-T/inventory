...
export async function PATCH(
  ...
  const {
    ...
    price_type, insurance_coverage_pct, sellingprice, unitcost,
  } = body;
  ...
  const result = await pool.query(
    `UPDATE items SET
      ...
      price_type             = COALESCE($14, price_type),
      insurance_coverage_pct = COALESCE($15, insurance_coverage_pct),
    ...
    [
      ...
      uom, manufacturer, description, barcode,
      min_level, reorder_level, max_level, controlled,
      price_type, insurance_coverage_pct ?? 0, sellingprice ?? null, unitcost ?? null,
      id,
    ]
  );
