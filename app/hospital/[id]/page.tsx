            ...
            <tbody>
              {items.map(item=>(
                <tr key={item.itemId}>
                  ...
                  <td style={s.td}><button onClick={()=>setItems(it=>it.filter(i=>i.itemId!==item.itemId))} style={{background:"#fee2e2",border:"none",borderRadius:6,padding:"4px 7px",cursor:"pointer"}}><Icon d={icons.trash} size={12} color="#dc2626"/></button></td>
                </tr>
              ))}
            </tbody>
          </table>
...
function TransferItemsList({transferId}:{transferId:string}){
  ...
  return<div>{items.map((item:any)=>(<div key={item.id} style={{display:"flex",justifyContent:"space-between",padding:"4px 0",borderBottom:"1px solid #f3f4f6",fontSize:13}}><span style={{fontWeight:600}}>{item.item_name}</span><span style={{color:"#6366f1",fontWeight:600}}>×{item.quantity}</span></div>))}</div>;
}
...
export default function DepartmentPage({params}:{params:Promise<{id:string}>}){
  ...
  return(
    <div style={s.page}>
      ...
      <div style={s.header}>
        <Link href="/hospital" style={{display:"flex",alignItems:"center",color:"#6b7280",textDecoration:"none"}}><Icon d={icons.back} size={15}/></Link>
        <div style={{width:1,height:20,background:"#e5e7eb"}}/>
        <div style={{width:32,height:32,background:dc.bg,borderRadius:8,display:"flex",alignItems:"center",justifyContent:"center"}}>{dc.icon}</div>
        <div>
        ...
        <div style={{marginLeft:"auto",display:"flex",gap:8}}>
          <button onClick={()=>fetchStock()} style={{...s.btn("ghost"),border:"1px solid #e5e7eb",display:"flex",alignItems:"center",gap:5}}><Icon d={icons.refresh} size={13} color="#374151"/></button>
        ...
        {tab==="dispense"&&(
          <div style={s.card}>
            <div style={{padding:"12px 16px",borderBottom:"1px solid #f3f4f6",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              ...
              <button onClick={()=>setDispenseModal(true)} style={{...s.btn("purple"),display:"flex",alignItems:"center",gap:6}}><Icon d={icons.plus} size={13} color="#fff"/> New Dispense</button>
            </div>
            <div style={{padding:40,textAlign:"center"}}>
              <div style={{fontSize:36,marginBottom:8}}>{dc.icon}</div>
        ...
        {tab==="request"&&(
          <div style={s.card}>
            ...
            {stock.filter(i=>parseInt(i.quantity||0)<=parseInt(i.reorder_level||0)).length===0?(
              <div style={{padding:40,textAlign:"center",color:"#9ca3af"}}><div style={{fontSize:32,marginBottom:8}}>✅</div><div style={{fontSize:14,fontWeight:600}}>All items are well stocked</div></div>
            ):(
              <div style={{overflowX:"auto"}}>
                <table style={{width:"100%",borderCollapse:"collapse"}}>
                ...
                <div key={t.id} style={{borderBottom:"1px solid #f3f4f6",padding:"16px 20px"}}>
                  ...
                  <div style={{display:"flex",gap:10,alignItems:"center"}}>
                    ...
                    <button disabled={receiveLoading===t.id} onClick={()=>handleReceive(t.id)}
                      ...
                      {receiveLoading===t.id?"Confirming...":"✓ Confirm Receipt"}
                    </button>
                  </div>
                </div>
              ))
